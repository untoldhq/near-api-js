...
export class WalletRedirect implements Wallet {
     ...
     * @hidden
     */
    async _requestSignTransactionsFromOldWalletConnection({ transactions, meta, callbackUrl }: RequestSignTransactionsOptions): Promise<void> {
        const currentUrl = new URL(window.location.href);
     ...
     */
    async _accessKeyMatchesTransaction(accessKey, receiverId: string, actions: Action[]): Promise<boolean> {
